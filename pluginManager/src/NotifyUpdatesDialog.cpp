	...
	switch (Message) 
	{
		...
		case WM_COMMAND : 
		{
			switch (wParam)
			{
				case IDC_IGNORE:
					{
						boostOrStd::shared_ptr<list<Plugin*> > selectedPlugins = _pluginListView.getSelectedPlugins();
						tstring pluginConfigFilename(_pluginList->getVariableHandler()->getVariable(_T("CONFIGDIR")));
						pluginConfigFilename.append(_T("\\PluginManager.ini"));
				...
				case IDC_UPDATE:
					{
						...
						ProgressDialog progress(_hInst, 
                            cancelToken,
							boostOrStd::bind(&PluginList::startInstall, _pluginList, _nppData._nppHandle, _1, &_pluginListView, TRUE, cancelToken));
