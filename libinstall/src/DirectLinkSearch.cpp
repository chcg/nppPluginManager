...
boostOrStd::shared_ptr<TCHAR> DirectLinkSearch::search(const TCHAR *filename)
{
	if (!filename || !(*filename))
	{
		boostOrStd::shared_ptr<TCHAR> empty;
		return empty;
	}
	...
	do 
	{
		...
		if (currentChar == filename[patternLength - checkOffset - 1])
		{
			...
			if (checkOffset == patternLength)
			{
				...
				{
					boostOrStd::shared_ptr<TCHAR> buffer(new TCHAR[currentPosition - realPosition + 2]);
